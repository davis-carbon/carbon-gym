...
// Fallback mock staff until DB is connected
const DEFAULT_STAFF: StaffColumn[] = [
...
const DEFAULT_APPOINTMENTS: CalendarAppointment[] = [
...
export function DayCalendar({ date, staff, appointments, onAppointmentClick }: DayCalendarProps) {
  const staffList = staff || DEFAULT_STAFF;
  const apptList = appointments || DEFAULT_APPOINTMENTS;
  const hours = Array.from({ length: END_HOUR - START_HOUR }, (_, i) => START_HOUR + i);

  return (
    <div className="rounded-xl border border-stone-200 bg-white overflow-hidden">
      {/* Staff header */}
      <div className="grid border-b border-stone-200 bg-stone-50" style={{ gridTemplateColumns: `64px repeat(${staffList.length}, 1fr)` }}>
        <div className="border-r border-stone-200 p-3" />
        {staffList.map((s) => (
          <div key={s.id} className="border-r border-stone-100 last:border-0 px-3 py-3 text-center">
      ...
      <div className="relative overflow-y-auto" style={{ maxHeight: "calc(100vh - 220px)" }}>
        <div
          ...
          style={{
            gridTemplateColumns: `64px repeat(${staffList.length}, 1fr)`,
        ...
        >
          ...
          <div className="border-r border-stone-200">
            {hours.map((hour) => (
              <div key={hour} className="relative border-b border-stone-100" style={{ height: HOUR_HEIGHT }}>
          ...
          {/* Staff columns */}
          {staffList.map((s) => {
            const staffAppointments = apptList.filter((a) => a.staffId === s.id);
            return (
              <div key={s.id} className="relative border-r border-stone-100 last:border-0">
                {hours.map((hour) => (
                  <div key={hour} className="border-b border-stone-100" style={{ height: HOUR_HEIGHT }} />
                ))}
                ...
                {staffAppointments.map((appt) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ top, height: Math.max(height - 2, 20), backgroundColor: bgColor }}
                    ...
                    >
                      ...
                      <p className="text-xs font-semibold text-stone-900 truncate">{appt.clientName}</p>
